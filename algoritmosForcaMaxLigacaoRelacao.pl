...
dfs2ForteNivel(Act,Dest,LA,Cam,N,C,SAux,Sum):-  C<N,
										   no(NAct,Act,_),
										   (ligacao(NAct,NX,F,_,FR,_);ligacao(NX,NAct,_,F,_,FR)), 
										   multiCriterio(F,FR,SOMA),			%RELACAO%
										   SAux1 is SAux + SOMA,
...
caminho_mais_forte_nivel(Orig,Dest,N,Cam,SumF):-
		get_time(Ti),
		(melhor_caminho_forte_nivel_relacao(Orig,Dest,N);true),
		retract(melhor_sol_forte_nivel(Cam,SumF,_)),
		get_time(Tf),
		T is Tf-Ti,
		write('Tempo de geracao da solucao:'),write(T),nl.

	melhor_caminho_forte_nivel_relacao(Orig,Dest,N):-
...
obterMaiorForcaLigacaoRelacao(Orig,N,FMax):-
									tamanhoRedeAdaptado(Orig,N,R),
									obterMaiorForcaLigacaoRelacao2(R,FMax).
...
obterMaiorForcaLigacaoRelacao2([_],0):- !.
obterMaiorForcaLigacaoRelacao2([H|T],FMax):-
									obterMaiorForcaLigacaoRelacao2(T,FMax1),
									obterMaiorForcaLigacaoRelacao3(H,T,FMax2),
									((FMax2>FMax1,!,FMax = FMax2);(FMax = FMax1)).


obterMaiorForcaLigacaoRelacao3(_,[],0):- !.
obterMaiorForcaLigacaoRelacao3(A,[B|T],FMax):-
									no(NumA,A,_),
									no(NumB,B,_),
									(ligacao(NumA,NumB,FAB,FBA,FRAB,FRBA);ligacao(NumB,NumA,FBA,FAB,FRBA,FRAB)),
									!,
									obterMaiorForcaLigacaoRelacao3(A,T,FMax1),
    								multiCriterio(FAB,FRAB,FmaxAB),
    								multiCriterio(FBA,FRBA,FmaxBA),
									((FmaxAB>FmaxBA,!,FMax2 = FmaxAB);(FMax2 = FmaxBA)),
									((FMax2>FMax1,!,FMax = FMax2);(FMax = FMax1)).
obterMaiorForcaLigacaoRelacao3(A,[_|T],FMax):-
									obterMaiorForcaLigacaoRelacao3(A,T,FMax).



multiCriterio(FL,FR,Resultado):- ((   FR =< -200, FR is -200) ; 
    							(   FR >= 200, FR is 200);true), FR1 is FR + 200, 
    							FR2 is FR1 / 400, FR3 is FR2 / 2, FL1 is FL/200,
    							Resultado is (FL1 + FR3) * 100.
...
aStar2(Dest,[(_,Ca,LA)|Outros],N,Cam,Custo):-
											...
											findall((CEX,CaX,[X|LA]),
													(C=<N,
													Dest\==Act,
													(ligacao(Act,X,CustoLigacaoX,_,CustoRelacaoX,_);ligacao(X,Act,_,CustoLigacaoX,_,CustoRelacaoX)),
													\+ member(X,LA),
                                                    multiCriterio(CustoLigacaoX,CustoLigacaoX,R),
													CaX is R + Ca,
...
calcula_custo([Act,X|L],S):-calcula_custo([X|L],S1), 
							(ligacao(Act,X,FL,_,FR,_);ligacao(X,Act,_,FL,_,FR)),
							multiCriterio(FL,FR,Soma),
							S is S1+Soma.
